...
/* check_ptr's functionality changed from project 2 to project 3 because it needs
   to guard against page faults. In project 2, the entire data, code, and stack
   segments were in memory and the stack couldn't grow. That is no longer the case
   in project 3, so there are several new situations to account for. When a user 
   calls a system call, the process is doing user-level work in the kernel. The 
   kernel can't page fault, so check_ptr ensures that page faults don't happen 
   during a syscall. The project 3 version of check_ptr behaves differently if a 
   page is not in the process' page table (pagedir_get_page answers whether it is). 
   If ptr points to unmapped memory, it will try to either load its page using the 
   supplemental page table or grow the stack (or potentailly neither, in which 
   case check_ptr returns false and its caller will exit with status code -1). 
   Because check_ptr may grow the stack, it needs to take an additional parameter 
   esp to determine if ptr is within 32 bytes of the stack pointer. check_ptr also
   pins every pointer to its frame so it cannot be evicted during the syscall. */ 
bool
...
/* I think I'll need to do pinning here */
int
...
/* Taking away all the added checks makes mmap_clean pass. I think I have a
   race somewhere that's fucking up exec or wait */
int
